...
import styles from "../../styles/pages/Puap-online-learning.module.scss";
...
export default function Puaponlinelearning({ data }) {
  return (
    <MainLayout
      ...
      // ------------------------------------------
      title={data.puaponlinelearningpagedata.meta.title}
      keywords={data.puaponlinelearningpagedata.meta.keywords}
      description={data.puaponlinelearningpagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.puaponlinelearningpagedata.firstscreen.firstscreentype ===
      "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.puaponlinelearningpagedata.firstscreen.h1title}
              h1subtitle={
                data.puaponlinelearningpagedata.firstscreen.h1subtitle
              }
              altimage={data.puaponlinelearningpagedata.firstscreen.imagealt}
              nameimage={data.puaponlinelearningpagedata.firstscreen.imagename}
              urlvideo={data.puaponlinelearningpagedata.firstscreen.urlvideo}
              titlevideo={
                data.puaponlinelearningpagedata.firstscreen.titlevideo
              }
              videobuttontitle={
                data.puaponlinelearningpagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
      ...
      {data.puaponlinelearningpagedata.landingmonolink.length !== 0 ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={
                data.puaponlinelearningpagedata.landingmonolink
              }
      ...
      {Object.keys(data.puaponlinelearningpagedata.faq).length !== 0 ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.puaponlinelearningpagedata.faq.faqtitle}
              faqdata={data.puaponlinelearningpagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.puaponlinelearningpagedata.landingmultilink.length !== 0 ? (
          ...
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={
                data.puaponlinelearningpagedata.landingmultilink
              }
            />
          </div>
          <div
      ...
      {data.puaponlinelearningpagedata.landingmultilink !== 0 ? (
        <FixLinks
          fixlinkdata={data.puaponlinelearningpagedata.landingmultilink}
        />
      ) : null}
    </MainLayout>
  );
...
export async function getStaticProps({ locale }) {
  ...
  const res = await fetch(
    `${process.env.FETCH_URL}:5000/api/puaponlinelearning_${lang}`
  );

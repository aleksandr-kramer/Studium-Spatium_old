...
export default function supervisor({ data }) {
  return (
    <MainLayout
      ...
      // ------------------------------------------
      title={data.supervisorpagedata.meta.title}
      keywords={data.supervisorpagedata.meta.keywords}
      description={data.supervisorpagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.supervisorpagedata.firstscreen.firstscreentype === "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.supervisorpagedata.firstscreen.h1title}
              h1subtitle={data.supervisorpagedata.firstscreen.h1subtitle}
              altimage={data.supervisorpagedata.firstscreen.imagealt}
              nameimage={data.supervisorpagedata.firstscreen.imagename}
              urlvideo={data.supervisorpagedata.firstscreen.urlvideo}
              titlevideo={data.supervisorpagedata.firstscreen.titlevideo}
              videobuttontitle={
                data.supervisorpagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
        </section>
      ) : data.supervisorpagedata.firstscreen.firstscreentype === "image" ? (
      ...
      {data.supervisorpagedata.landingmonolink.length !== 0 ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={data.supervisorpagedata.landingmonolink}
      ...
      {Object.keys(data.supervisorpagedata.faq).length !== 0 ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.supervisorpagedata.faq.faqtitle}
              faqdata={data.supervisorpagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.supervisorpagedata.landingmultilink.length !== 0 ? (
        <section className={styles.main__landingmultilink}>
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={data.supervisorpagedata.landingmultilink}
            />
          </div>
          <div
      ...
      {data.supervisorpagedata.landingmultilink !== 0 ? (
        <FixLinks fixlinkdata={data.supervisorpagedata.landingmultilink} />
      ) : null}
    </MainLayout>
  );
}
...
export async function getStaticProps({ locale }) {
  ...
  const res = await fetch(
    `${process.env.FETCH_URL}:5000/api/supervisor_${lang}`
  );

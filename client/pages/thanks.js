...
export default function Thanks({ data }) {
  return (
    <MainLayout
      ...
      // ------------------------------------------
      title={data.thankspagedata.meta.title}
      keywords={data.thankspagedata.meta.keywords}
      description={data.thankspagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.thankspagedata.firstscreen.firstscreentype === "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.thankspagedata.firstscreen.h1title}
              h1subtitle={data.thankspagedata.firstscreen.h1subtitle}
              altimage={data.thankspagedata.firstscreen.imagealt}
              nameimage={data.thankspagedata.firstscreen.imagename}
              urlvideo={data.thankspagedata.firstscreen.urlvideo}
              titlevideo={data.thankspagedata.firstscreen.titlevideo}
              videobuttontitle={
                data.thankspagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
        </section>
      ) : data.thankspagedata.firstscreen.firstscreentype === "image" ? (
      ...
      {data.thankspagedata.landingmonolink.length !== 0 ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={data.thankspagedata.landingmonolink}
      ...
      {Object.keys(data.thankspagedata.faq).length !== 0 ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.thankspagedata.faq.faqtitle}
              faqdata={data.thankspagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.thankspagedata.landingmultilink.length !== 0 ? (
        <section className={styles.main__landingmultilink}>
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={data.thankspagedata.landingmultilink}
            />
          </div>
          <div
...
export async function getStaticProps({ locale }) {
  const lang = locale;
  const res = await fetch(`${process.env.FETCH_URL}:5000/api/thanks_${lang}`);

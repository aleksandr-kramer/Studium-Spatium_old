...
export default function Authschool({ data }) {
  return (
    <MainLayout
      // ------------------------------------------
      ...
      // ------------------------------------------
      title={data.authschoolpagedata.meta.title}
      keywords={data.authschoolpagedata.meta.keywords}
      description={data.authschoolpagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.authschoolpagedata.firstscreen.firstscreentype === "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.authschoolpagedata.firstscreen.h1title}
              h1subtitle={data.authschoolpagedata.firstscreen.h1subtitle}
              altimage={data.authschoolpagedata.firstscreen.imagealt}
              nameimage={data.authschoolpagedata.firstscreen.imagename}
              urlvideo={data.authschoolpagedata.firstscreen.urlvideo}
              titlevideo={data.authschoolpagedata.firstscreen.titlevideo}
              videobuttontitle={
                data.authschoolpagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
        </section>
      ) : data.authschoolpagedata.firstscreen.firstscreentype === "image" ? (
      ...
      <section className={styles.main__contact}>
        <div className={styles.main__contact__container}>
          <FeedbackComponent
            title={data.servicedata.request[2].title}
            feedbackcomponenttextdata={data.servicedata.request[2].text}
            feedbackcomponentabouticondata={
              data.servicedata.request[2].abouticonspoint
            }
          >
            <FormChildrenRequestPresentation
              inputformchildrenrequestpresentationdata={
                data.servicedata.request[2].form.input
              }
              textareatrue={
                data.servicedata.request[2].form.textarea.istextarea
              }
              textareaplaceholder={
                data.servicedata.request[2].form.textarea.placeholder
              }
              buttonattachfiletrue={
                data.servicedata.request[2].form.buttonattachfile
                  .isbuttonattachfile
              }
              buttonattachfiletype={
                data.servicedata.request[2].form.buttonattachfile.fieldtype
              }
              buttonattachfiletext={
                data.servicedata.request[2].form.buttonattachfile.buttontext
              }
              buttonattachfilename={
                data.servicedata.request[2].form.buttonattachfile.filenametext
              }
              buttonsendtype={
                data.servicedata.request[2].form.buttonsend.fieldtype
              }
              buttonsendtext={
                data.servicedata.request[2].form.buttonsend.buttontext
              }
            />
          </FeedbackComponent>
        </div>
      ...
      {data.authschoolpagedata.islandingmonolink ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={data.authschoolpagedata.landingmonolink}
      ...
      {data.authschoolpagedata.isfaq ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.authschoolpagedata.faq.faqtitle}
              faqdata={data.authschoolpagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.authschoolpagedata.islandingmultilink ? (
        <section className={styles.main__landingmultilink}>
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={data.authschoolpagedata.landingmultilink}
            />
          </div>
          <div
      ...
      {data.authschoolpagedata.islandingmultilink ? (
        <FixLinks fixlinkdata={data.authschoolpagedata.landingmultilink} />
      ) : null}
    </MainLayout>
  );
}
...
export async function getStaticProps({ locale }) {
  ...
  const res = await fetch(
    `${process.env.FETCH_URL}:5000/api/authschool_${lang}`
  );

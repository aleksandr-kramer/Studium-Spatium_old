...
export default function Courses({ data }) {
  return (
    <MainLayout
      ...
      // ------------------------------------------
      title={data.coursespagedata.meta.title}
      keywords={data.coursespagedata.meta.keywords}
      description={data.coursespagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.coursespagedata.firstscreen.firstscreentype === "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.coursespagedata.firstscreen.h1title}
              h1subtitle={data.coursespagedata.firstscreen.h1subtitle}
              altimage={data.coursespagedata.firstscreen.imagealt}
              nameimage={data.coursespagedata.firstscreen.imagename}
              urlvideo={data.coursespagedata.firstscreen.urlvideo}
              titlevideo={data.coursespagedata.firstscreen.titlevideo}
              videobuttontitle={
                data.coursespagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
        </section>
      ) : data.coursespagedata.firstscreen.firstscreentype === "image" ? (
      ...
      {data.coursespagedata.landingmonolink.length !== 0 ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={data.coursespagedata.landingmonolink}
      ...
      {Object.keys(data.coursespagedata.faq).length !== 0 ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.coursespagedata.faq.faqtitle}
              faqdata={data.coursespagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.coursespagedata.landingmultilink.length !== 0 ? (
        <section className={styles.main__landingmultilink}>
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={data.coursespagedata.landingmultilink}
            />
          </div>
          <div
      ...
      {data.coursespagedata.landingmultilink !== 0 ? (
        <FixLinks fixlinkdata={data.coursespagedata.landingmultilink} />
      ) : null}
    </MainLayout>
  );
}
...
export async function getStaticProps({ locale }) {
  const lang = locale;
  const res = await fetch(`${process.env.FETCH_URL}:5000/api/courses_${lang}`);

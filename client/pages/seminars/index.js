...
export default function Seminars({ data }) {
  return (
    <MainLayout
      // ------------------------------------------
      ...
      // ------------------------------------------
      title={data.seminarspagedata.meta.title}
      keywords={data.seminarspagedata.meta.keywords}
      description={data.seminarspagedata.meta.description}
    >
      {/* Блок первого экрана для каждой страницы (выбор из трёх вариантов)*/}
      {/* Начало */}

      {data.seminarspagedata.firstscreen.firstscreentype === "video" ? (
        <section className={styles.main__firstscreenvideo}>
          <div className={styles.main__firstscreenvideo__container}>
            <FirstScreenVideo
              h1title={data.seminarspagedata.firstscreen.h1title}
              h1subtitle={data.seminarspagedata.firstscreen.h1subtitle}
              altimage={data.seminarspagedata.firstscreen.imagealt}
              nameimage={data.seminarspagedata.firstscreen.imagename}
              urlvideo={data.seminarspagedata.firstscreen.urlvideo}
              titlevideo={data.seminarspagedata.firstscreen.titlevideo}
              videobuttontitle={
                data.seminarspagedata.firstscreen.videobuttontitle
              }
              socialnetworkdata={data.socialnetworkdata}
            />
          </div>
        </section>
      ) : data.seminarspagedata.firstscreen.firstscreentype === "image" ? (
      ...
      {data.seminarspagedata.landingmonolink.length !== 0 ? (
        <section className={styles.main__landingmonolink}>
          <div className={styles.main__landingmonolink__container}>
            <LandingMonoLink
              landingmonolinkdata={data.seminarspagedata.landingmonolink}
      ...
      {Object.keys(data.seminarspagedata.faq).length !== 0 ? (
        <section className={styles.main__faq}>
          <div className={styles.main__faq__container}>
            <Faq
              title={data.seminarspagedata.faq.faqtitle}
              faqdata={data.seminarspagedata.faq.questionanswer}
            />
          </div>
      ...
      {data.seminarspagedata.landingmultilink.length !== 0 ? (
        <section className={styles.main__landingmultilink}>
          <div className={styles.main__landingmultilink__container}>
            <LandingMultiLink
              listlandingmultilink={data.seminarspagedata.landingmultilink}
            />
          </div>
          <div
      ...
      {data.seminarspagedata.landingmultilink !== 0 ? (
        <FixLinks fixlinkdata={data.seminarspagedata.landingmultilink} />
      ) : null}
    </MainLayout>
  );
}
...
export async function getStaticProps({ locale }) {
  const lang = locale;
  const res = await fetch(`${process.env.FETCH_URL}:5000/api/seminars_${lang}`);

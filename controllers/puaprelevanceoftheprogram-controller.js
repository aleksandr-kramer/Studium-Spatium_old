...
const getPagePuaprelevanceoftheprogramEn = async (req, res) => {
  try {
    ...
    const puaprelevanceoftheprogrampagedata = await Page_en.findOne({
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      puaprelevanceoftheprogrampagedata,
      articledata,
...
const getPagePuaprelevanceoftheprogramEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puaprelevanceoftheprogrampagedata = await Page_es.findOne({
      pagename: "puap-relevance-of-the-program",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuaprelevanceoftheprogramRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puaprelevanceoftheprogrampagedata = await Page_ru.findOne({
      pagename: "puap-relevance-of-the-program",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

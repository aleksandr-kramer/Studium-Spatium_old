...
const getPagePuaptheoreticaltrainingEn = async (req, res) => {
  try {
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      puaptheoreticaltrainingpagedata,
      articledata,
...
const getPagePuaptheoreticaltrainingEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puaptheoreticaltrainingpagedata = await Page_es.findOne({
      pagename: "puap-theoretical-training",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuaptheoreticaltrainingRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puaptheoreticaltrainingpagedata = await Page_ru.findOne({
      pagename: "puap-theoretical-training",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

...
const getPageSeminarcriteriaassessmentEn = async (req, res) => {
  try {
    ...
    const seminarcriteriaassessmentpagedata = await Page_en.findOne({
      pagename: "seminar-criteria-assessment",
    })
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      seminarcriteriaassessmentpagedata,
    });
  } catch (error) {
    console.log(error);
  }
...
const getPageSeminarcriteriaassessmentEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const seminarcriteriaassessmentpagedata = await Page_es.findOne({
      pagename: "seminar-criteria-assessment",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPageSeminarcriteriaassessmentRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const seminarcriteriaassessmentpagedata = await Page_ru.findOne({
      pagename: "seminar-criteria-assessment",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

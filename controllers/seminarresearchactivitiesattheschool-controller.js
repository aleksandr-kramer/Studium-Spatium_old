...
const getPageSeminarresearchactivitiesattheschoolEn = async (req, res) => {
  try {
    ...
    const seminarresearchactivitiesattheschoolpagedata = await Page_en.findOne({
      pagename: "seminar-research-activities-at-the-school",
    })
...
const getPageSeminarresearchactivitiesattheschoolEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const seminarresearchactivitiesattheschoolpagedata = await Page_es.findOne({
      pagename: "seminar-research-activities-at-the-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPageSeminarresearchactivitiesattheschoolRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const seminarresearchactivitiesattheschoolpagedata = await Page_ru.findOne({
      pagename: "seminar-research-activities-at-the-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

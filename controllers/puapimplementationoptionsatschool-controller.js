...
const getPagePuapimplementationoptionsatschoolEn = async (req, res) => {
  try {
    ...
    const puapimplementationoptionsatschoolpagedata = await Page_en.findOne({
      pagename: "puap-implementation-options-at-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
    const articledata = await Article_en.findOne({
      _id: "64308071bb5bf2345c2677c6",
    });
    const infopagenavigationdata = await Page_en.find({
      thematiccategory: "schools-and-teachers",
      pagename: { $ne: "puap-implementation-options-at-school" },
    });
    res.json({
      logodata,
      socialnetworkdata,
      navigationdata,
      footerdata,
      puapimplementationoptionsatschoolpagedata,
      articledata,
...
const getPagePuapimplementationoptionsatschoolEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puapimplementationoptionsatschoolpagedata = await Page_es.findOne({
      pagename: "puap-implementation-options-at-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuapimplementationoptionsatschoolRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puapimplementationoptionsatschoolpagedata = await Page_ru.findOne({
      pagename: "puap-implementation-options-at-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

...
const getPagePuaprequestapresentationoftheprogramEn = async (req, res) => {
  try {
    ...
    const puaprequestapresentationoftheprogrampagedata = await Page_en.findOne({
      pagename: "puap-request-a-presentation-of-the-program",
    })
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      puaprequestapresentationoftheprogrampagedata,
    });
  } catch (error) {
    console.log(error);
  }
...
const getPagePuaprequestapresentationoftheprogramEs = async (req, res) => {
  try {
    ...
    const puaprequestapresentationoftheprogrampagedata = await Page_es.findOne({
      pagename: "puap-request-a-presentation-of-the-program",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuaprequestapresentationoftheprogramRu = async (req, res) => {
  try {
    ...
    const puaprequestapresentationoftheprogrampagedata = await Page_ru.findOne({
      pagename: "puap-request-a-presentation-of-the-program",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

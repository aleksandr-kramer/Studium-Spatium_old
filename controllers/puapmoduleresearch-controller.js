...
const getPagepuapmoduleresearchEn = async (req, res) => {
  try {
    ...
    const puapmoduleresearchpagedata = await Page_en.findOne({
      pagename: "puap-module-research",
    })
...
const getPagepuapmoduleresearchEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puapmoduleresearchpagedata = await Page_es.findOne({
      pagename: "puap-module-research",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagepuapmoduleresearchRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puapmoduleresearchpagedata = await Page_ru.findOne({
      pagename: "puap-module-research",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

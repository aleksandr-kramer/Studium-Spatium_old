...
const getPageAuthportfolioEn = async (req, res) => {
  try {
    ...
    const authportfoliopagedata = await Page_en.findOne({
      pagename: "auth-portfolio",
    })
...
const getPageAuthportfolioEs = async (req, res) => {
  try {
    ...
    const authportfoliopagedata = await Page_es.findOne({
      pagename: "auth-portfolio",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPageAuthportfolioRu = async (req, res) => {
  try {
    ...
    const authportfoliopagedata = await Page_ru.findOne({
      pagename: "auth-portfolio",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

...
const getPagePuaphowtoimplementtheprogramatschoolEn = async (req, res) => {
  try {
    ...
    const puaphowtoimplementtheprogramatschoolpagedata = await Page_en.findOne({
      pagename: "puap-how-to-implement-the-program-at-school",
    })
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      puaphowtoimplementtheprogramatschoolpagedata,
    });
  } catch (error) {
    console.log(error);
  }
...
const getPagePuaphowtoimplementtheprogramatschoolEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puaphowtoimplementtheprogramatschoolpagedata = await Page_es.findOne({
      pagename: "puap-how-to-implement-the-program-at-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuaphowtoimplementtheprogramatschoolRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puaphowtoimplementtheprogramatschoolpagedata = await Page_ru.findOne({
      pagename: "puap-how-to-implement-the-program-at-school",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

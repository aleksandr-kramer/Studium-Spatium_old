...
const getPagecontactEn = async (req, res) => {
  try {
    ...
    const contactpagedata = await Page_en.findOne({
      pagename: "contact",
    })
...
const getPagecontactEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const contactpagedata = await Page_es.findOne({
      pagename: "contact",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagecontactRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const contactpagedata = await Page_ru.findOne({
      pagename: "contact",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

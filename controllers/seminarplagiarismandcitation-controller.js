...
const getPageseminarplagiarismandcitationEn = async (req, res) => {
  try {
    ...
    const seminarplagiarismandcitationpagedata = await Page_en.findOne({
      pagename: "seminar-plagiarism-and-citation",
    })
    ...
    res.json({
      ...
      navigationdata,
      footerdata,
      seminarplagiarismandcitationpagedata,
    });
  } catch (error) {
    console.log(error);
  }
...
const getPageseminarplagiarismandcitationEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const seminarplagiarismandcitationpagedata = await Page_es.findOne({
      pagename: "seminar-plagiarism-and-citation",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPageseminarplagiarismandcitationRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const seminarplagiarismandcitationpagedata = await Page_ru.findOne({
      pagename: "seminar-plagiarism-and-citation",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

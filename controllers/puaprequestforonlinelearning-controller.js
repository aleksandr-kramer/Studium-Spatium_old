...
const getPagePuaprequestforonlinelearningEn = async (req, res) => {
  try {
    ...
    const puaprequestforonlinelearningpagedata = await Page_en.findOne({
      pagename: "puap-request-for-online-learning",
    })
...
const getPagePuaprequestforonlinelearningEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const puaprequestforonlinelearningpagedata = await Page_es.findOne({
      pagename: "puap-request-for-online-learning",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPagePuaprequestforonlinelearningRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const puaprequestforonlinelearningpagedata = await Page_ru.findOne({
      pagename: "puap-request-for-online-learning",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");

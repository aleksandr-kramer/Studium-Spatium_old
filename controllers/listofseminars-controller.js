...
const getPageListofseminarsEn = async (req, res) => {
  try {
    ...
    const listofseminarspagedata = await Page_en.findOne({
      pagename: "list-of-seminars",
    })
...
const getPageListofseminarsEs = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    const socialnetworkdata = await Socialnetwork.find({ locale: "en" });
    ...
    const listofseminarspagedata = await Page_es.findOne({
      pagename: "list-of-seminars",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
...
const getPageListofseminarsRu = async (req, res) => {
  try {
    const logodata = await Logo.findOne({ _id: "641859e21127a88a8a58f32d" });
    ...
    const listofseminarspagedata = await Page_ru.findOne({
      pagename: "list-of-seminars",
    })
      .populate("landingmonolink")
      .populate("landingmultilink");
